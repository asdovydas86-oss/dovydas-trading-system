...
| [design/SWING_SETUP_ENGINE_V1.md](design/SWING_SETUP_ENGINE_V1.md) | Approved design for Swing Setup Engine v1 (Milestone AR): the first directional-interpretation layer, why CONTEXT/SETUP form the thesis and EXECUTION only confirms, the three-independent-family tally that forbids single-indicator direction, the regime-as-gate-not-vote and Decision-Context-INSUFFICIENT-forecloses-unconditionally rules, deterministic stop/target/R:R selection with no fabricated entry, and why probability stays `NOT_CALIBRATED` | Design — implemented by [ADR-0028](adr/ADR-0028-directional-interpretation-boundary.md) |
| [reviews/SWING_SETUP_ENGINE_V1_REVIEW.md](reviews/SWING_SETUP_ENGINE_V1_REVIEW.md) | Independent review of Swing Setup Engine v1: a fresh adversarial pass found **three P1s** — a same-bar dual break tie always resolving toward the LOWER side, no recency bound on a confirming break, and a one-CLI-flag interval collision that defeated the ≥2-independent-families guarantee — all three fixed and regression-tested; 24 mutation probes across two passes plus an independent 3-mutation spot-check, 24 detected, 0 survivors | Review record — no P0, three P1 found and fixed, one P3 found and fixed |
