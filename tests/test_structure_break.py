...
def level(
    ...
    return PriceLevel(price=price, side=side, origin=origin(origin_index, label))
...
def test_a_break_projects_every_derived_fact() -> None:
    ...
    got = StructureBreak(crossing=crossing, eligible_from=7)
    assert got.index == 8
...
def test_the_model_stores_exactly_two_fields() -> None:
    assert set(StructureBreak.__dataclass_fields__) == {"crossing", "eligible_from"}
...
def test_a_break_is_immutable() -> None:
    got = StructureBreak(crossing=event(level(103.0, LevelSide.UPPER, 5), 8), eligible_from=7)
...
def test_equality_and_hashing_are_structural() -> None:
    upper = level(103.0, LevelSide.UPPER, 5)
    crossing = event(upper, 8)
    a = StructureBreak(crossing=crossing, eligible_from=7)
    b = StructureBreak(crossing=crossing, eligible_from=7)
    assert a == b
    assert hash(a) == hash(b)
    assert len({a, b}) == 1
    assert a != StructureBreak(crossing=crossing, eligible_from=8)
...
def test_a_break_round_trips_through_pickle() -> None:
    ...
    got = derive_structure_breaks(levels, crossings, confirmation_bars=RB)
...
def test_a_break_cannot_be_built_from_a_non_close_crossing(
    ...
    with pytest.raises(ValueError) as excinfo:
        StructureBreak(crossing=event(upper, 8, kind=kind), eligible_from=7)
...
def test_a_break_cannot_be_built_from_an_already_beyond_crossing() -> None:
    ...
    with pytest.raises(ValueError) as excinfo:
        StructureBreak(crossing=crossing, eligible_from=7)
...
def test_a_break_cannot_be_built_from_an_unprovenanced_level() -> None:
    ...
    with pytest.raises(ValueError) as excinfo:
        StructureBreak(crossing=event(bare, 8), eligible_from=0)
    assert str(excinfo.value) == (
...
def test_a_break_cannot_precede_its_own_eligibility() -> None:
    upper = level(103.0, LevelSide.UPPER, 5)
    with pytest.raises(ValueError) as excinfo:
        StructureBreak(crossing=event(upper, 6), eligible_from=7)
    assert str(excinfo.value) == (
...
def test_eligible_from_cannot_precede_the_level_origin() -> None:
    upper = level(103.0, LevelSide.UPPER, 5)
    with pytest.raises(ValueError) as excinfo:
        StructureBreak(crossing=event(upper, 8), eligible_from=4)
    assert str(excinfo.value) == (
        "eligible_from (4) cannot precede the level origin index (5)"
    )


def test_a_negative_eligible_from_is_rejected() -> None:
    upper = level(103.0, LevelSide.UPPER, 5)
    with pytest.raises(ValueError) as excinfo:
        StructureBreak(crossing=event(upper, 8), eligible_from=-1)
    assert str(excinfo.value) == "eligible_from cannot be negative, got -1"


@pytest.mark.parametrize(
    "kwargs,message",
    [
        (dict(crossing="x"), "crossing must be a LevelCrossingEvent, got str"),
        (dict(eligible_from="7"), "eligible_from must be an int, got str"),
        (dict(eligible_from=True), "eligible_from must be an int, got bool"),
    ],
)
def test_model_type_validation(kwargs: dict, message: str) -> None:
    base = dict(crossing=event(level(103.0, LevelSide.UPPER, 5), 8), eligible_from=7)
    with pytest.raises(TypeError) as excinfo:
        StructureBreak(**{**base, **kwargs})
    assert str(excinfo.value) == message
...
def test_upward_is_not_named_bullish_anywhere() -> None:
    ...
    levels, crossings = pipeline(series(UPWARD_ROWS))
    got = derive_structure_breaks(levels, crossings, confirmation_bars=RB)[0]
    assert got.side is LevelSide.UPPER
...
def test_no_break_when_nothing_closes_beyond_a_reference() -> None:
    ...
    levels, crossings = pipeline(series(rows))
    assert derive_structure_breaks(levels, crossings, confirmation_bars=RB) == ()
...
def test_a_touch_or_wick_breach_never_breaks_structure(kind: CrossingKind) -> None:
    ...
    got = derive_structure_breaks(
        [upper], [event(upper, 8, kind=kind)], confirmation_bars=RB
    )
    assert got == ()
...
def test_a_close_breach_of_the_reference_breaks_structure() -> None:
    upper = level(103.0, LevelSide.UPPER, 5)
    got = derive_structure_breaks([upper], [event(upper, 8)], confirmation_bars=RB)
    assert len(got) == 1
...
def test_a_wick_beyond_that_closes_back_inside_is_a_rejection_not_a_break() -> None:
    ...
    assert derive_structure_breaks([upper], [wick], confirmation_bars=RB) == ()
...
def test_there_is_no_way_to_configure_the_qualifying_kind() -> None:
    ...
    assert parameters == {"levels", "crossings", "confirmation_bars"}
...
def test_an_equal_low_derived_level_breaks_when_price_closes_beyond_it() -> None:
    ...
    got = derive_structure_breaks(
        [equal_low], [event(equal_low, 8)], confirmation_bars=RB
    )
...
def test_every_label_can_produce_a_break(label: StructuralSwingLabel) -> None:
    ...
    got = derive_structure_breaks([lvl], [event(lvl, 8)], confirmation_bars=RB)
...
def test_a_level_is_not_eligible_before_its_confirmation_bar() -> None:
    ...
    for index in range(5, 7):
        assert derive_structure_breaks(
            [upper], [event(upper, index)], confirmation_bars=RB
        ) == ()
    assert len(derive_structure_breaks([upper], [event(upper, 7)], confirmation_bars=RB)) == 1
...
def test_eligible_from_is_the_origin_plus_the_confirmation_delay() -> None:
    ...
    for bars in (0, 1, 2, 5):
        got = derive_structure_breaks(
            [upper], [event(upper, 20)], confirmation_bars=bars
        )
...
def test_only_the_most_recent_eligible_level_is_the_reference() -> None:
    ...
    got = derive_structure_breaks([old, new], crossings, confirmation_bars=RB)
...
def test_a_superseded_level_never_breaks_structure() -> None:
    ...
    old = level(100.0, LevelSide.UPPER, 3)
    new = level(110.0, LevelSide.UPPER, 8)
    got = derive_structure_breaks([old, new], [event(old, 20)], confirmation_bars=RB)
    assert got == ()
...
def test_the_reference_is_most_recent_not_most_extreme() -> None:
    ...
    got = derive_structure_breaks(
        [high, lower_high], [event(lower_high, 20)], confirmation_bars=RB
    )
...
def test_before_any_level_is_eligible_there_is_no_reference() -> None:
    ...
    assert breaks_mod._reference(
        breaks_mod._levels_by_side([upper], RB)[LevelSide.UPPER], 6
    ) is None
...
def test_sides_are_tracked_independently() -> None:
    ...
    got = derive_structure_breaks(
        [upper, lower], [event(upper, 20), event(lower, 21)], confirmation_bars=RB
    )
...
def test_the_earliest_qualifying_crossing_wins_regardless_of_input_order() -> None:
    ...
    for order in itertools.permutations([event(upper, 12), event(upper, 8), event(upper, 10)]):
        got = derive_structure_breaks([upper], list(order), confirmation_bars=RB)
        assert [b.index for b in got] == [8]
...
def test_a_broken_level_stops_producing_breaks_and_no_newer_level_replaces_it() -> None:
    upper = level(103.0, LevelSide.UPPER, 5)
    got = derive_structure_breaks(
        [upper], [event(upper, i) for i in range(8, 30)], confirmation_bars=RB
    )
    assert len(got) == 1
...
def test_a_break_is_never_invalidated() -> None:
    ...
    extended = derive_structure_breaks(
        levels, list(crossings) + [event(levels[0], 40)], confirmation_bars=RB
    )
...
def test_an_already_beyond_crossing_never_breaks_structure() -> None:
    ...
    assert derive_structure_breaks([upper], [beyond], confirmation_bars=RB) == ()
...
def test_the_mechanism_is_carried_through_for_the_consumer() -> None:
    ...
    for mechanism in (CrossingMechanism.WITHIN_RANGE, CrossingMechanism.GAPPED_BEYOND):
        got = derive_structure_breaks(
            [upper], [event(upper, 8, mechanism=mechanism)], confirmation_bars=RB
        )
...
def test_one_bar_can_break_both_sides() -> None:
    ...
    got = derive_structure_breaks([upper, lower], crossings, confirmation_bars=RB)
    assert [(b.index, b.side) for b in got] == [
        (20, LevelSide.UPPER),
        (20, LevelSide.LOWER),
    ]
...
def test_two_levels_on_one_side_sharing_an_origin_index_are_rejected() -> None:
    ...
    with pytest.raises(StructureBreakInputError) as excinfo:
        derive_structure_breaks([a, b], [], confirmation_bars=RB)
    assert str(excinfo.value) == (
...
def test_two_levels_on_different_sides_may_share_an_origin_index() -> None:
    ...
    got = derive_structure_breaks(
        [upper, lower], [event(upper, 20)], confirmation_bars=RB
    )
    assert len(got) == 1
...
def test_an_unprovenanced_level_is_rejected_even_with_no_crossings() -> None:
    ...
    with pytest.raises(StructureBreakInputError):
        derive_structure_breaks([PriceLevel(105.0, LevelSide.UPPER)], [], confirmation_bars=RB)
...
def test_a_crossing_referencing_an_unknown_level_is_rejected() -> None:
    ...
    with pytest.raises(StructureBreakInputError) as excinfo:
        derive_structure_breaks([known], [event(unknown, 20)], confirmation_bars=RB)
    assert str(excinfo.value) == (
...
def test_an_equal_valued_but_distinct_level_object_is_still_unknown() -> None:
    ...
    with pytest.raises(StructureBreakInputError):
        derive_structure_breaks([known], [event(twin, 20)], confirmation_bars=RB)
...
def test_duplicate_provenance_across_sides_is_accepted_and_ranked() -> None:
    ...
    ranked = breaks_mod._levels_by_side([upper, lower], RB)
...
def test_two_levels_at_one_price_with_different_origins_are_both_kept() -> None:
    ...
    got = derive_structure_breaks(
        [first, second], [event(second, 20)], confirmation_bars=RB
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda: derive_structure_breaks("levels", [], confirmation_bars=RB),
            "levels must be a sequence of PriceLevel, got str",
        ),
        (
            lambda: derive_structure_breaks([], "crossings", confirmation_bars=RB),
            "crossings must be a sequence of LevelCrossingEvent, got str",
        ),
        (
            lambda: derive_structure_breaks([1], [], confirmation_bars=RB),
            "levels[0] must be a PriceLevel, got int",
        ),
        (
            lambda: derive_structure_breaks(
                [level(100.0, LevelSide.UPPER, 5)], [1], confirmation_bars=RB
            ),
            "crossings[0] must be a LevelCrossingEvent, got int",
        ),
        (
            lambda: derive_structure_breaks([], [], confirmation_bars="2"),
            "confirmation_bars must be an int, got str",
        ),
        (
            lambda: derive_structure_breaks([], [], confirmation_bars=True),
            "confirmation_bars must be an int, got bool",
        ),
    ],
)
...
def test_a_negative_confirmation_delay_is_rejected() -> None:
    with pytest.raises(ValueError) as excinfo:
        derive_structure_breaks([], [], confirmation_bars=-1)
    assert str(excinfo.value) == "confirmation_bars cannot be negative, got -1"


def test_confirmation_bars_is_required_and_keyword_only() -> None:
    import inspect

    parameter = inspect.signature(derive_structure_breaks).parameters["confirmation_bars"]
...
def test_zero_confirmation_bars_is_permitted() -> None:
    upper = level(103.0, LevelSide.UPPER, 5)
    got = derive_structure_breaks([upper], [event(upper, 5)], confirmation_bars=0)
    assert [b.eligible_from for b in got] == [5]
...
def test_empty_levels_and_crossings() -> None:
    assert derive_structure_breaks([], [], confirmation_bars=RB) == ()


def test_levels_but_no_crossings() -> None:
    assert derive_structure_breaks(
        [level(100.0, LevelSide.UPPER, 5)], [], confirmation_bars=RB
    ) == ()
...
def test_crossings_but_no_levels_is_rejected_not_silently_empty() -> None:
    ...
    with pytest.raises(StructureBreakInputError):
        derive_structure_breaks([], [event(upper, 20)], confirmation_bars=RB)


def test_an_empty_result_is_an_immutable_tuple() -> None:
    assert isinstance(derive_structure_breaks([], [], confirmation_bars=RB), tuple)
...
def test_permuting_the_level_input_changes_nothing() -> None:
    ...
    levels, crossings = pipeline(series(rows))
    reference = derive_structure_breaks(levels, crossings, confirmation_bars=RB)
    rng = random.Random(17)
    for _ in range(10):
        shuffled = list(levels)
        rng.shuffle(shuffled)
        assert derive_structure_breaks(shuffled, crossings, confirmation_bars=RB) == reference
...
def test_permuting_the_crossing_input_changes_nothing() -> None:
    ...
    for _ in range(10):
        ...
        rng.shuffle(shuffled)
        assert derive_structure_breaks(levels, shuffled, confirmation_bars=RB) == reference
...
def test_reversing_both_inputs_changes_nothing() -> None:
    ...
    levels, crossings = pipeline(series(rows))
    assert derive_structure_breaks(
        list(reversed(levels)), list(reversed(crossings)), confirmation_bars=RB
    ) == derive_structure_breaks(levels, crossings, confirmation_bars=RB)
...
def test_duplicated_crossing_events_change_nothing() -> None:
    ...
    reference = derive_structure_breaks(levels, crossings, confirmation_bars=RB)
    assert derive_structure_breaks(
        levels, list(crossings) * 3, confirmation_bars=RB
    ) == reference
...
def _prefix_violations(
    ...
    full = derive_structure_breaks(levels, crossings, confirmation_bars=right_bars)
    violations = 0
    for n in range(len(rows) + 1):
        plevels, pcrossings = pipeline(series(rows[:n]), right_bars)
        got = derive_structure_breaks(plevels, pcrossings, confirmation_bars=right_bars)
...
def test_prefix_stability_on_the_real_fixture() -> None:
    ...
    levels, crossings = pipeline(real)
    full = derive_structure_breaks(levels, crossings, confirmation_bars=RB)
    for n in range(len(real.candles) + 1):
        ...
        got = derive_structure_breaks(plevels, pcrossings, confirmation_bars=RB)
        assert [(b.index, b.side, b.level.price) for b in got] == [
...
def test_the_documented_counterexample_is_stable_under_the_shipped_rule() -> None:
    ...
    prefix = derive_structure_breaks(plevels, pcrossings, confirmation_bars=RB)
...
def test_adding_later_crossings_cannot_change_earlier_breaks() -> None:
    ...
    for n in range(1, len(ordered)):
        short = derive_structure_breaks(levels, ordered[:n], confirmation_bars=RB)
        longer = derive_structure_breaks(levels, ordered[: n + 1], confirmation_bars=RB)
...
def test_replay_is_deterministic() -> None:
    ...
    levels, crossings = pipeline(series(rows))
    first = derive_structure_breaks(levels, crossings, confirmation_bars=RB)
    for _ in range(5):
        assert derive_structure_breaks(levels, crossings, confirmation_bars=RB) == first
...
def test_a_rebuilt_equal_pipeline_replays_identically() -> None:
    ...
    assert derive_structure_breaks(
        a_levels, a_crossings, confirmation_bars=RB
    ) == derive_structure_breaks(b_levels, b_crossings, confirmation_bars=RB)
...
def test_the_safe_pipeline_end_to_end() -> None:
    ...
    got = contextual_structure_breaks(levels, crossings, confirmation_bars=RB)
...
def test_an_instrument_mismatch_is_rejected() -> None:
    ...
    with pytest.raises(SeriesIdentityMismatchError) as excinfo:
        contextual_structure_breaks(levels, eth, confirmation_bars=RB)
    assert str(excinfo.value) == (
...
def test_a_timeframe_mismatch_is_rejected() -> None:
    ...
    with pytest.raises(SeriesIdentityMismatchError) as excinfo:
        contextual_structure_breaks(levels, other, confirmation_bars=RB)
    assert str(excinfo.value) == (
...
def test_a_mismatch_is_rejected_before_any_derivation() -> None:
    ...
    with pytest.raises(SeriesIdentityMismatchError):
        contextual_structure_breaks(bad_levels, other, confirmation_bars=RB)
...
def test_a_non_empty_result_also_carries_the_levels_identity_object() -> None:
    ...
    got = contextual_structure_breaks(levels, rewrapped, confirmation_bars=RB)
    assert got.values
    assert got.identity is levels.identity


def test_eligibility_is_computed_in_exactly_one_place() -> None:
    """`_levels_by_side` owns `origin.index + confirmation_bars`; nothing restates it."""
    upper = level(103.0, LevelSide.UPPER, 5)
    for bars in (0, 1, 2, 4):
        ranked = breaks_mod._levels_by_side([upper], bars)[LevelSide.UPPER]
...
def test_a_bare_payload_where_an_envelope_belongs_is_rejected() -> None:
    levels, crossings = _contextual(series(UPWARD_ROWS))
    with pytest.raises(TypeError) as excinfo:
        contextual_structure_breaks((), crossings, confirmation_bars=RB)
    assert str(excinfo.value) == "levels must be a ContextualSeries, got tuple"
    with pytest.raises(TypeError) as excinfo:
        contextual_structure_breaks(levels, (), confirmation_bars=RB)
...
def test_context_free_and_context_aware_payloads_are_identical(name: str) -> None:
    ...
    bare = derive_structure_breaks(levels, crossings, confirmation_bars=RB)
    clevels, ccrossings = _contextual(candles)
    wrapped = contextual_structure_breaks(clevels, ccrossings, confirmation_bars=RB)
...
def test_every_break_references_a_supplied_level() -> None:
    ...
    levels, crossings = pipeline(series(rows))
    for got in derive_structure_breaks(levels, crossings, confirmation_bars=RB):
...
def test_every_break_satisfies_all_five_conjuncts() -> None:
    ...
    for b in got:
        ...
        ranked = breaks_mod._levels_by_side(levels, RB)[b.side]
...
def test_a_larger_confirmation_delay_never_invents_a_break_of_a_newer_level() -> None:
    ...
    levels, crossings = pipeline(series(rows))
    for bars in (2, 3, 4, 5):
        for b in derive_structure_breaks(levels, crossings, confirmation_bars=bars):
...
def test_repeated_execution_produces_identical_results() -> None:
    ...
    levels, crossings = pipeline(series(rows))
    results = [
        derive_structure_breaks(levels, crossings, confirmation_bars=RB) for _ in range(5)
    ]
...
def test_an_exhaustive_small_reference_space_is_consistent() -> None:
    ...
    for o1, o2 in itertools.product(range(1, 6), repeat=2):
        ...
        for at in range(1, 14):
            for target in (a, b):
                got = derive_structure_breaks(
                    [a, b], [event(target, at)], confirmation_bars=RB
                )
                eligible = [
...
def test_a_future_choch_layer_needs_only_the_break_sequence() -> None:
    ...
    levels, crossings = pipeline(series(rows))
    breaks = derive_structure_breaks(levels, crossings, confirmation_bars=RB)
...
def test_a_break_is_self_describing_without_its_inputs() -> None:
    ...
    levels, crossings = pipeline(series(rows))
    for b in derive_structure_breaks(levels, crossings, confirmation_bars=RB):
        described = (
